...
class RandomCard:
    ...
    def card(self):
        ...
        # Таким образом получаю случайные индексы в строке карточки, которые заменим на пробел
        ...
        for string in all_number:
            ...
            for i in range(len(one_string)):
                if len(one_string[i]) == 1:
                    one_string[i] = one_string[i].ljust(2, ' ') #добавляем пробел для ровной отрисовки карточки
...
class Game:
    ...
    @staticmethod                                             #Поиск в карточке и зачеркивание бочонка
    ...
    def start(self):
        ...
        while True:
            ...
            print('\n'
                  'Зачеркнуто номеров: {}; {}'.format(point_player, point_enemy))
            ...
            if Game.check_keg(keg, all_number_enemy):
                ...
                if point_enemy == 15:
                    print('********************'
                          ...
                          '********************')
                    break
            answer = input('Зачеркнуть БОЧОНОК?(y/n) для выхода нажмите q ')
            if answer == 'y':
